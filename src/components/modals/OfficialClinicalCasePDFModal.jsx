...
export const OfficialClinicalCasePDFModal = ({ isOpen, onClose, clinicalCase, student, preceptor, college }) => {
  ...
  useEffect(() => {
    const loadModules = async () => {
      ...
      setLoading(true);
      const res = await fetchCaseModuleStatusesFromSupabase(clinicalCase.id);
      if (res.success) {
      ...
      <div className="space-y-4 text-xs">
        ...
        <div className="max-h-[65vh] overflow-y-auto border border-slate-200 dark:border-slate-800 rounded-2xl p-4 bg-slate-100 dark:bg-slate-900/50">
          {loading ? (
            ...
            <div id="official-clinical-case-pdf-container" className="bg-white text-slate-900 p-8 rounded-xl shadow-md space-y-6 max-w-3xl mx-auto font-sans relative">
              
              {/* WATERMARK */}
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-[0.03] select-none">
                <span className="text-8xl font-black rotate-[-30deg] uppercase tracking-widest text-slate-900">
                  PharmDVerse Approved
                </span>
              </div>
              ...
              <div className="flex items-center justify-between pb-4 border-b-2 border-slate-900">
                <div className="flex items-center gap-3">
                  {college?.college_logo_url || college?.logoUrl ? (
                    <img src={college.college_logo_url || college.logoUrl} alt="College Logo" className="w-14 h-14 object-contain" />
                  ) : (
                    <div className="w-14 h-14 bg-emerald-700 text-white font-black text-lg flex items-center justify-center rounded-xl">
                      {college?.initials || 'PHARMD'}
                    </div>
                  )}
                  <div>
                    <h1 className="text-base font-black text-slate-900 uppercase tracking-tight">
                      {college?.college_name || 'COLLEGE OF PHARMACY'}
                    </h1>
                    <p className="text-[10px] font-semibold text-slate-600">DEPARTMENT OF PHARMACY PRACTICE</p>
                    <p className="text-[10px] font-bold text-emerald-700">Official Pharm.D Clinical Case Logbook Record</p>
                  </div>
                </div>

                <div className="text-right">
                  <span className="inline-block px-3 py-1 rounded-md bg-emerald-700 text-white text-[10px] font-black uppercase tracking-wider">
                    APPROVED RECORD
                  </span>
                  <p className="text-xs font-mono font-bold mt-1 text-slate-900">{caseId}</p>
                </div>
              </div>
              ...
              {profile.id && (
                <div className="space-y-2 pt-2 border-t border-slate-200">
                  <h3 className="text-xs font-black uppercase tracking-wider text-emerald-800 flex items-center justify-between border-b pb-1">
              ...
              {counselling.id && (
                <div className="space-y-2 pt-2 border-t border-slate-200">
                  <h3 className="text-xs font-black uppercase tracking-wider text-teal-800 flex items-center justify-between border-b pb-1">
              ...
              {/* MODULE 3: PHARMACIST INTERVENTION (IF PRESENT) */}
              {intervention.id && (
                <div className="space-y-2 pt-2 border-t border-slate-200">
                  <h3 className="text-xs font-black uppercase tracking-wider text-indigo-800 flex items-center justify-between border-b pb-1">
              ...
              {/* MODULE 4: DRUG INFORMATION REQUEST (IF PRESENT) */}
              {dir.id && (
                <div className="space-y-2 pt-2 border-t border-slate-200">
                  <h3 className="text-xs font-black uppercase tracking-wider text-cyan-800 flex items-center justify-between border-b pb-1">
              ...
              {/* MODULE 5: ADR DOCUMENTATION (IF PRESENT) */}
              {adr.id && (
                <div className="space-y-2 pt-2 border-t border-slate-200">
                  <h3 className="text-xs font-black uppercase tracking-wider text-amber-800 flex items-center justify-between border-b pb-1">
              ...
              {/* OFFICIAL PRECEPTOR APPROVAL BOX & FOOTER */}
