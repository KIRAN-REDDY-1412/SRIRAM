...
import { ShieldAlert, User, FileText, Plus, Trash2, CheckSquare, Square, Save, Eye, Send, ArrowLeft, Loader2, CheckCircle2, AlertTriangle, BookOpen, Layers } from 'lucide-react';
import { fetchPharmacistInterventionByCaseIdFromSupabase, saveOrUpdatePharmacistInterventionInSupabase } from '../../services/supabaseService';
...
export const PharmacistInterventionFormView = ({ clinicalCase, student, onBack }) => {
  ...
  const [prescriptionDetails, setPrescriptionDetails] = useState([
    { s_no: 1, drug_name: 'Tab. Metformin', dose_frequency: '500 mg BD' },
    { s_no: 2, drug_name: 'Tab. Telmisartan', dose_frequency: '40 mg OD' }
  ]);

  // 3. Prescription Problems
  const [prescriptionProblems, setPrescriptionProblems] = useState(['Drug Interaction', 'High Dose']);
  ...
  const [actionTaken, setActionTaken] = useState(['Discussion with prescriber', 'Drug information reference consulted']);
  const [actionTakenOther, setActionTakenOther] = useState('');
  const [recommendations, setRecommendations] = useState(['Dose', 'Schedule']);
  ...
  const [referencesText, setReferencesText] = useState('Micromedex / Lexicomp Clinical Drug Information');
  const [followUp, setFollowUp] = useState('Monitored daily during clinical pharmacy ward rounds.');
  ...
  useEffect(() => {
    const loadInterventionData = async () => {
      ...
      const res = await fetchPharmacistInterventionByCaseIdFromSupabase(clinicalCase.id);
      if (res.success && res.intervention) {
        ...
        setAge(item.age || '');
        setSex(item.sex || 'M');
        setDateOfIntervention(item.date_of_intervention || new Date().toISOString().split('T')[0]);
        setIpOpNo(item.ip_op_no || '');
        setWard(item.ward || clinicalCase.ward_unit || '');
        setPresentDiagnosis(item.present_diagnosis || '');

        if (item.prescription_details && Array.isArray(item.prescription_details)) setPrescriptionDetails(item.prescription_details);
  ...
  const handleSaveIntervention = async (newStatus = 'Draft') => {
    ...
    if (!patientName.trim() || !descriptionOfProblem.trim()) {
      setFormError('Please enter Patient Name and Description of Problem.');
      return;
    }
    ...
    const payload = {
      ...
      accepted,
      changed,
      reasons_if_no: (!accepted || !changed) ? reasonsIfNo : null,
    ...
    <div className="space-y-6 animate-fadeIn max-w-5xl mx-auto">
      ...
      <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-xs">
          <div className="sm:col-span-2">
            ...
            <input type="text" required value={patientName} onChange={(e) => setPatientName(e.target.value)} placeholder="Full Patient Name" className="w-full h-[44px] px-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900 text-slate-900 dark:text-white font-bold" />
          </div>
          ...
          <div>
            ...
            <input type="date" value={dateOfIntervention} onChange={(e) => setDateOfIntervention(e.target.value)} className="w-full h-[44px] px-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900 text-slate-900 dark:text-white font-mono" />
          </div>

          <div>
            <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">IP / OP No.</label>
            <input type="text" value={ipOpNo} onChange={(e) => setIpOpNo(e.target.value)} placeholder="IP/OP Number" className="w-full h-[44px] px-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900 text-slate-900 dark:text-white font-mono font-bold" />
          </div>

          <div>
            <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Ward</label>
            <input type="text" value={ward} onChange={(e) => setWard(e.target.value)} placeholder="Ward Unit" className="w-full h-[44px] px-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900 text-slate-900 dark:text-white" />
          </div>

          <div className="sm:col-span-2">
            <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Present Diagnosis</label>
            <input type="text" value={presentDiagnosis} onChange={(e) => setPresentDiagnosis(e.target.value)} placeholder="Clinical diagnosis..." className="w-full h-[44px] px-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900 text-slate-900 dark:text-white" />
          </div>
        </div>
      </div>
                ...
                <tr key={index}>
                  ...
                  <td className="p-3">
                    <input
                      ...
                      className="w-full h-9 px-3 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white font-semibold"
                    />
                  </td>
                  <td className="p-3">
                    <input
                      ...
                      className="w-full h-9 px-3 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white font-mono"
                    />
                  </td>
      ...
      <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-4">
        ...
        <div>
          ...
          <input type="text" value={prescriptionProblemOther} onChange={(e) => setPrescriptionProblemOther(e.target.value)} placeholder="Other prescription problems..." className="w-full h-9 px-3.5 text-xs rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900 text-slate-900 dark:text-white" />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Detailed description of clinical drug problem..."
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-3">
          ...
          <div className="space-y-2 text-xs">
            ...
            <div>
              <input type="text" value={actionTakenOther} onChange={(e) => setActionTakenOther(e.target.value)} placeholder="Others (specify)..." className="w-full h-9 px-3 text-xs rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900 text-slate-900 dark:text-white" />
            </div>
          </div>
        </div>
        ...
        <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-3">
          ...
          <div className="space-y-2 text-xs">
            ...
            <div>
              <input type="text" value={recommendationOther} onChange={(e) => setRecommendationOther(e.target.value)} placeholder="Others (Specify)..." className="w-full h-9 px-3 text-xs rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900 text-slate-900 dark:text-white" />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2 text-xs">
          ...
          {(!accepted || !changed) && (
            <div>
              ...
              <input type="text" value={reasonsIfNo} onChange={(e) => setReasonsIfNo(e.target.value)} placeholder="Reasons..." className="w-full h-9 px-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900 text-slate-900 dark:text-white" />
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
          <div>
            ...
            <textarea
              ...
              placeholder="Clinical pharmacology references..."
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Daily follow-up notes..."
              className="w-full p-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900 text-slate-900 dark:text-white"
            />
          </div>
        </div>
