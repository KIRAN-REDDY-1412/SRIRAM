 ...
 * Respects strict form boundaries, renders 100% of submitted fields, and ensures watermark visibility.
 */
export const generateClinicalCasePPTX = async ({
  ...
  // Watermark helper with high visibility styling
  ...
  // FORM 1: PATIENT PROFILE DOCUMENTATION (SLIDES 2, 3, 4)
  if (norm.isProfileCompleted) {
    ...
    const profileRows = [
      ...
      [{ text: 'Measurements & Allergies', options: { fontFace, fontSize: 10, bold: true } }, { text: `Ht: ${norm.demographics.height} | Wt: ${norm.demographics.weight} | Allergies: ${norm.demographics.allergyDrugs}`, options: { fontFace, fontSize: 10 } }],
    ...
    const vitalsRows = norm.vitals.length > 0
      ? norm.vitals.map(v => [
    ...
    // SLIDE 4: MEDICATIONS & DISCHARGE SUMMARY
    const slide4 = pptx.addSlide();
    addWatermark(slide4);
    slide4.addText('Prescribed Medication Profile', {
      x: startX, y: 0.3, w: contentW, h: 0.4,
      fontFace, fontSize: titleFontSize, bold: true, color: primaryColor
    });
    ...
    const drugRows = norm.drugs.length > 0
      ...
      : [
          [{ text: '1', options: { fontFace, fontSize: 9, align: 'center' } }, { text: 'Symptomatic Medication', options: { fontFace, fontSize: 9, bold: true } }, { text: 'As prescribed', options: { fontFace, fontSize: 9 } }, { text: 'OD', options: { fontFace, fontSize: 9 } }, { text: 'Symptomatic', options: { fontFace, fontSize: 9 } }]
        ];
  ...
  if (norm.isCounsellingCompleted) {
    ...
    const counsellingRows = [
      ...
      [{ text: 'Duration / Representative', options: { fontFace, fontSize: 9, bold: true } }, { text: `${norm.counselling.timeTaken} ${norm.counselling.representativeReasons ? `(${norm.counselling.representativeReasons})` : ''}`, options: { fontFace, fontSize: 9 } }],
      [{ text: 'Disease Counselled', options: { fontFace, fontSize: 9, bold: true } }, { text: norm.counselling.diseaseCounselled, options: { fontFace, fontSize: 9 } }],
      [{ text: 'Key Focus Points', options: { fontFace, fontSize: 9, bold: true } }, { text: norm.counselling.pointsCovered, options: { fontFace, fontSize: 9 } }],
      [{ text: 'Barriers & Action Taken', options: { fontFace, fontSize: 9, bold: true } }, { text: `${norm.counselling.majorBarriers} ${norm.counselling.barrierOvercome ? `— ${norm.counselling.barrierOvercome}` : ''}`, options: { fontFace, fontSize: 9 } }],
  ...
  if (norm.isInterventionCompleted) {
    ...
    const interventionRows = [
      ...
      [{ text: 'Problem Identified', options: { fontFace, fontSize: 9, bold: true } }, { text: norm.intervention.prescriptionProblems, options: { fontFace, fontSize: 9 } }],
      [{ text: 'Action & Recommendation', options: { fontFace, fontSize: 9, bold: true } }, { text: `${norm.intervention.actionsTaken} — ${norm.intervention.recommendations}`, options: { fontFace, fontSize: 9 } }],
      [{ text: 'Significance & Acceptance', options: { fontFace, fontSize: 9, bold: true } }, { text: `Significance: ${norm.intervention.significanceLevel} | Status: ${norm.intervention.physicianAcceptance}`, options: { fontFace, fontSize: 9, color: emeraldColor, bold: true } }],
  ...
  if (norm.isDirCompleted) {
    ...
    const dirRows = [
      ...
      [{ text: 'Category of Enquiry', options: { fontFace, fontSize: 9, bold: true } }, { text: norm.dir.questionCategory, options: { fontFace, fontSize: 9 } }],
      [{ text: 'Details of Query', options: { fontFace, fontSize: 9, bold: true } }, { text: norm.dir.detailsOfEnquiry, options: { fontFace, fontSize: 9 } }],
      [{ text: 'Response Provided', options: { fontFace, fontSize: 9, bold: true } }, { text: norm.dir.informationProvided, options: { fontFace, fontSize: 9 } }],
      [{ text: 'Student Signature', options: { fontFace, fontSize: 9, bold: true } }, { text: `Digitally Signed by ${studentName} (${rollNumber})`, options: { fontFace, fontSize: 9 } }],
      [{ text: 'Preceptor Signature', options: { fontFace, fontSize: 9, bold: true } }, { text: `Verified & Approved by ${preceptorName}`, options: { fontFace, fontSize: 9, bold: true, color: primaryColor } }]
    ];
  ...
  if (norm.isAdrCompleted) {
    ...
    const adrRows = [
      ...
      [{ text: 'ADR Number & Onset Date', options: { fontFace, fontSize: 9, bold: true } }, { text: `Log No: ${norm.adr.adrNumber} | Onset: ${norm.adr.onsetDate}`, options: { fontFace, fontSize: 9 } }],
      [{ text: 'Suspected Drug & Reaction', options: { fontFace, fontSize: 9, bold: true } }, { text: `${norm.adr.reactionCategory} — ${norm.adr.reactionTitle}`, options: { fontFace, fontSize: 9 } }],
      [{ text: 'Causality (Naranjo) & Severity', options: { fontFace, fontSize: 9, bold: true } }, { text: `Causality: ${norm.adr.naranjoCausality} | Severity: ${norm.adr.reactionSeverity} (${norm.adr.reactionSeriousness})`, options: { fontFace, fontSize: 9 } }],
      [{ text: 'Dechallenge & Rechallenge', options: { fontFace, fontSize: 9, bold: true } }, { text: `Dechallenge: ${norm.adr.dechallengeInfo} | Rechallenge: ${norm.adr.rechallengeInfo}`, options: { fontFace, fontSize: 9 } }],
      [{ text: 'Student Signature', options: { fontFace, fontSize: 9, bold: true } }, { text: `Digitally Signed by ${studentName} (${rollNumber})`, options: { fontFace, fontSize: 9 } }],
      [{ text: 'Preceptor Signature', options: { fontFace, fontSize: 9, bold: true } }, { text: `Verified & Approved by ${preceptorName}`, options: { fontFace, fontSize: 9, bold: true, color: primaryColor } }]
    ];
